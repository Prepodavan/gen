...
func parseAll(conf *Config) (*Suite, error) {
	...
	for _, pkgPath := range conf.RequestedPackages {
		...
		if astPkg, ok := pkg.Origin().(*ast.Package); ok {
			if len(astPkg.Files) > 0 {
				suite.Packages[pkgPath] = astPkg
				for filePath, _ := range astPkg.Files {
					if conf.RequestedPackagesPaths == nil {
						conf.RequestedPackagesPaths = make(map[string]string)
					}
					conf.RequestedPackagesPaths[pkgPath] = filepath.Dir(filePath)
					break
				}
			}
		}
		err = parseGoType(pkg, imp, suite)
		if err != nil {
			return nil, err
		}
	}
...
func parseGoType(in gotype.Type, gotypeImporter *gotype.Importer, out *Suite) error {
	var ok bool
	var pkg *ast.Package
	pkg, ok = in.Origin().(*ast.Package)
	if !ok {
		return nil
	}
	for _, f := range pkg.Files {
		for _, is := range f.Imports {
			...
			t, err := gotypeImporter.Import(pv, pv)
			if err != nil {
				return err
			}
			if t.IsGoroot() {
			...
			if pkg, ok := t.Origin().(*ast.Package); ok {
				out.Packages[pv] = pkg
			}
			err = parseGoType(t, gotypeImporter, out)
			if err != nil {
				return err
			}
		}
...
func getPackages(directive string, conf *Config) ([]*Package, error) {
	// get the AST
	suite, err := parseAll(conf)
	if err != nil {
		return nil, err
	}

	loadPkgs := make(map[string]bool)
	for pkgPath, _ := range suite.Packages {
	...
	var pkgs []*Package
	for astPkgPath, astPkg := range suite.Packages {
		pkg := &Package{Package: prog.Package(astPkgPath).Pkg, fset: suite.Fset, Types: []Type{}}
		pkgs = append(pkgs, pkg)

		specs := getTaggedComments(astPkg, directive)

		for s, c := range specs {
			pointer, tags, err := parse(suite.Fset, c, directive)
				...
				for i, val := range tag.Values {
					for _, item := range val.typeParameters {
						...
						if evalErr != nil {
							...
							tc.ignored = conf.IgnoreTypeCheckErrors
							tc.addPos(suite.Fset, item.pos+c.Slash)
						}
			...
			typ.test = test(strings.HasSuffix(suite.Fset.Position(s.Pos()).Filename, "_test.go"))
